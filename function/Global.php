...
function formatAngka(int|float $value, $simbol = null)
{
    // if float has decimals, keep 2 decimals; else 0 decimals (backward compatible)
    $isFloat = is_float($value) || str_contains((string) $value, '.');
    if ($isFloat) {
        $decimals = 2;
        // trim trailing .00 if not needed? keep decimals for currency
        if (fmod((float) $value, 1) == 0.0) {
            $decimals = 0;
        }
        return $simbol.number_format((float) $value, $decimals, ',', '.');
    }
    return $simbol.number_format((int) $value, 0, ',', '.');
}

function formatRupiah(mixed $value, bool $withPrefix = true): string
{
    if ($value === null || $value === '') return '-';
    $num = (float) $value;
    $prefix = $withPrefix ? 'Rp ' : '';
    // hilangkan ,00 kalau bulat
    if (fmod($num, 1) == 0.0) {
...
function uploadFile($file, string $folder = 'uploads', array $options = []): string
{
    ...
    // Move file to storage — coba storeAs, fallback ke move manual (Windows junction / permission)
    ...
    // Fallback jika storeAs tidak menulis file (mis. Windows/Junction race)
    if (! file_exists($fullPath)) {
        ...
        try {
            ...
            throw new InvalidArgumentException('Gagal menyimpan file: '.$e->getMessage());
        }
        if (! file_exists($fullPath)) {
            throw new InvalidArgumentException('Gagal menyimpan file — file tidak ditemukan setelah upload.');
        }
    }

    // Ensure file permissions are safe (guard jika file belum ter-tulis karena race / disk)
    if (file_exists($fullPath)) {
        @chmod($fullPath, 0644);
    }
...
/**
 * Format custom field key to human label: no_stnk -> No Stnk / No STNK
 */
function customFieldLabel(string $key): string
{
    return ucwords(str_replace('_', ' ', $key));
}

/**
 * Filter custom fields: keep only non-empty values, returns [] if not array.
 *
 * @param  array|null  $fields  e.g. $model->aset_custom_fields (already cast to array)
 * @return array<string,mixed>
 */
function getCustomFields(?array $fields): array
{
...
/**
 * Render custom fields as HTML grid (mobile card).
 * Usage in blade: {!! renderCustomFields($table->aset_custom_fields) !!}
 * Or with custom wrapper: {!! renderCustomFields($table->aset_custom_fields, 'grid-cols-3') !!}
 */
function renderCustomFields(?array $fields, string $gridClass = 'grid-cols-2'): string
{
    $filtered = getCustomFields($fields);
    ...
    foreach ($filtered as $key => $val) {
        // Handle array value (e.g. select multiple) -> join
        if (is_array($val)) {
            $val = implode(', ', $val);
        }
        // Format date-like values if parsable
        if (is_string($val) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $val)) {
            try { $val = formatDate($val); } catch (\Throwable $e) {}
        }
        $label = e(customFieldLabel($key));
        $value = e((string) $val);
        $html .= '<div><p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">'.$label.'</p><p class="text-xs font-medium text-on-surface">'.$value.'</p></div>';
    }
