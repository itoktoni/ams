<?php /** @var App\Models\Faktur $table */ ?>
            ...
            <x-slot:mobile>
                ...
                <div class="p-3 space-y-3" id="mBody">
                    ...
                    <div class="border border-outline-variant rounded-xl p-4 bg-surface-container-lowest shadow-sm" data-id="{{ $table->field_primary }}">
                        <p class="text-sm font-bold text-on-surface truncate mb-3">{{ $table->faktur_nomor }}</p>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Tanggal</p>
                                <p class="text-xs font-medium text-primary truncate">{{ $table->faktur_tanggal ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Status</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->faktur_status ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Total</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->faktur_total ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">ID</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->faktur_id }}</p>
                            </div>
                        </div>
