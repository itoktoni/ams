<?php /** @var App\Models\SukuCadang $table */ ?>
            ...
            <x-slot:mobile>
                ...
                <div class="p-3 space-y-3" id="mBody">
                    ...
                    <div class="border border-outline-variant rounded-xl p-4 bg-surface-container-lowest shadow-sm" data-id="{{ $table->field_primary }}">
                        <p class="text-sm font-bold text-on-surface truncate mb-3">{{ $table->suku_cadang_nama }}</p>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Kode</p>
                                <p class="text-xs font-medium text-primary truncate">{{ $table->suku_cadang_kode }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Harga</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->suku_cadang_harga ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Stok Min</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->suku_cadang_stok_minimum ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">ID</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->suku_cadang_id }}</p>
                            </div>
                        </div>
