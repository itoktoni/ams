            ...
            <x-slot:mobile>
                ...
                <div class="p-3 space-y-3" id="mBody">
                    ...
                    <div class="border border-outline-variant rounded-xl p-4 bg-surface-container-lowest shadow-sm" data-id="{{ $table->field_primary }}">
                        <p class="text-sm font-bold text-on-surface truncate mb-3">{{ $table->daftar_tunggu_id_aset ?? 'Daftar Tunggu #'.$table->field_primary }}</p>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Status</p>
                                <p class="text-xs font-medium text-primary truncate">{{ $table->daftar_tunggu_status }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Durasi</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->daftar_tunggu_durasi }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Mulai</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->daftar_tunggu_tanggal_mulai }}</p>
                            </div>
                            <div>
