<?php /** @var App\Models\PermintaanSukuCadang $table */ ?>
        ...
        <x-table>
            ...
            <x-slot:body>
                ...
                <tr>
                    ...
                    <td>{{ $table->hasSukuCadang?->suku_cadang_nama ?? '-' }}@if($table->hasSukuCadang?->suku_cadang_kode) <span class="text-[11px] text-on-surface-variant">({{ $table->hasSukuCadang->suku_cadang_kode }})</span>@endif</td>
                    @foreach ($model::$sortColumns as $column)
                    <td>
                        @if(str_contains($column, 'tanggal') && $table->$column)
                            {{ formatDate($table->$column) }}
                        @elseif(str_contains($column, 'harga') || str_contains($column, 'subtotal') || str_contains($column, 'total') || str_contains($column, 'biaya') || str_contains($column, 'nominal'))
                            {{ is_numeric($table->$column) ? formatRupiah($table->$column) : ($table->$column ?? '-') }}
                        @elseif(str_contains($column, 'jumlah'))
                            {{ $table->$column !== null ? formatQty($table->$column) : '-' }}
                        @else
                            {{ $table->$column ?? '-' }}
                        @endif
                    </td>
                    ...
                    <div class="bg-white rounded-2xl border border-outline-variant/20 shadow-sm overflow-hidden" data-id="{{ $table->field_primary }}" onclick="mToggle(this)">
                        <div class="p-3.5">
                            ...
                            <div class="mt-3">
                                ...
                                <p class="text-xs text-primary font-medium mt-1 truncate">{{ $table->hasSukuCadang?->suku_cadang_nama ?? '-' }}@if($table->hasSukuCadang?->suku_cadang_kode) <span class="text-on-surface-variant">({{ $table->hasSukuCadang->suku_cadang_kode }})</span>@endif</p>
                            </div>
                                    ...
                                    <div class="bg-surface-container-low/70 rounded-xl px-2.5 py-2">
                                        ...
                                        <p class="text-xs font-medium text-on-surface mt-1 truncate">
                                            @if(str_contains($col, 'tanggal') && $val)
                                                {{ formatDate($val) }}
                                            @elseif(str_contains($col, 'harga') || str_contains($col, 'subtotal') || str_contains($col, 'tarif') || str_contains($col, 'total') || str_contains($col, 'biaya') || str_contains($col, 'nominal'))
                                                {{ is_numeric($val) ? formatRupiah($val) : ($val ?? '-') }}
                                            @elseif(str_contains($col, 'jumlah'))
                                                {{ $val !== null ? formatQty($val) : '-' }}
