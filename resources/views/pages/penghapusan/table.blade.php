            ...
            <x-slot:mobile>
                ...
                <div class="p-3 space-y-3" id="mBody">
                    ...
                    <div class="border border-outline-variant rounded-xl p-4 bg-surface-container-lowest shadow-sm" data-id="{{ $table->field_primary }}">
                        <p class="text-sm font-bold text-on-surface truncate mb-3">{{ $table->penghapusan_nomor }}</p>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Aset</p>
                                <p class="text-xs font-medium text-primary truncate">{{ $table->penghapusan_id_aset }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Status</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->penghapusan_status }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] text-on-surface-variant uppercase tracking-wide mb-0.5">Triase</p>
                                <p class="text-xs font-medium text-on-surface">{{ $table->penghapusan_triase }}</p>
                            </div>
                            <div>
