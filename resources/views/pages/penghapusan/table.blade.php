<?php /** @var App\Models\Penghapusan $table */ ?>
        ...
        <x-table>
            ...
            <x-slot:body>
                ...
                <tr>
                    ...
                    <td class="min-w-[160px]">{{ $table->hasAset?->aset_nama ? $table->hasAset->aset_nama.' — '.$table->hasAset->aset_kode : ($table->penghapusan_id_aset ?? '-') }}</td>
                    @foreach ($model::$sortColumns as $column)
                    <td>
                        @if(str_contains($column, 'tanggal') && $table->$column)
                            {{ formatDate($table->$column) }}
                        @elseif(str_contains($column, 'nilai') || str_contains($column, 'harga') || str_contains($column, 'biaya') || str_contains($column, 'nominal') || str_contains($column, 'total'))
                    ...
                    <div class="bg-white rounded-2xl border border-outline-variant/20 shadow-sm overflow-hidden" data-id="{{ $table->field_primary }}" onclick="mToggle(this)">
                        <div class="p-3.5">
                            ...
                            <div class="mt-3">
                                ...
                                <p class="text-xs text-on-surface-variant mt-1 truncate">{{ $table->hasAset?->aset_nama ? $table->hasAset->aset_nama.' — '.$table->hasAset->aset_kode : ($table->penghapusan_id_aset ?? '') }}</p>
                            </div>
                                ...
                                @foreach(array_slice($model::$sortColumns, 0, 4) as $col)
                                    ...
                                    <div class="bg-surface-container-low/70 rounded-xl px-2.5 py-2">
                                        ...
                                        <p class="text-xs font-medium text-on-surface mt-1 truncate">
                                            ...
                                            @elseif(str_contains($col, 'nilai') || str_contains($col, 'harga') || str_contains($col, 'tarif') || str_contains($col, 'total') || str_contains($col, 'biaya') || str_contains($col, 'nominal'))
                                                {{ is_numeric($val) ? formatRupiah($val) : ($val ?? '-') }}
                                            @elseif(str_contains($col, 'status') || str_contains($col, 'kondisi') || str_contains($col, 'level') || str_contains($col, 'urgensi') || str_contains($col, 'triase'))
                                                {{ $val ?? '-' }}
                                            @else
                                                {{ Str::limit($val ?? '-', 24) }}
                                            @endif
